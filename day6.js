const input = `wdcmlzfnugqtvjbsahi
easrkmocxbpjgi
...
for (let i = 0; i < questionGroups.length; i++) {
	groupArray.push(questionGroups[i].split(/\r?\n/));
}
...
function day6part1() {
	...
	for (let i = 0; i < groupArray.length; i ++) {
		...
		for (let j = 0; j < groupArray[i].length; j++) {
			for (let k = 0; k < groupArray[i][j].length; k++) {
				if (!letterArray.includes(groupArray[i][j][k])) {
					letterArray.push(groupArray[i][j][k]);
				}
			}
		}
...
function day6part2() {
	let count = 0;
	const groupArray2 = groupArray.slice(0, 2);
	let row;
	for (let i = 0; i < groupArray.length; i ++) {
		row = groupArray[i];
		let tempLetter = new Set();
		let letterArray = [];
		for (let j = 0; j < row.length; j++) {
			for (let k = 0; k < row[j].length; k++) {
				letterArray.push(row[j][k]);
			}
			letterArray.forEach(l => {
				let c = counts(l, letterArray);
				if (row.length === c) {
					tempLetter.add(l);
				}
			});
		count+= tempLetter.size;
		}
	}
	return count;
}

console.log("day6part1: ", day6part1());
console.log("day6part2: ", day6part2());
